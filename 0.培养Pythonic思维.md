...
所谓C风格的格式字符串指的是这种：`print('%d' % 0xf)`，即使用`%`作为格式化操作符来替换左边的格式字符串的方式，这种写法来自C语言的printf函数，
还有其他语言也参照了这种写法。除了`%d`，还有其他格式替换符如`%s %x %f`等。
...
在Python3中添加了高级字符串格式化机制，它的表达能力比老式C风格的格式字符串要强，且不再使用`%`。那就是内置的format函数以及str.format()函数，
这两个函数用法基本一致，如下：
```shell
>>> format(1234.5678, ',.2f')
'1,234.57'
...
'1,234.57'
>>> '{1}, {1} = {0}'.format('value', 'key') // 可以用数字指代要替换的值的索引，这个特性同时解决了上面的缺点1，3
'key, key = value'
...
但是，这个format函数并没有解决上面的缺点2，4，所以下面要介绍一种能够解决上述全部问题的方法那就是Python3.6引入的 **f-string**，叫做插值格式字符串（interpolated format string，简称f-string）。
这个方案讲格式字符串的表达能力发挥到了极致，它彻底解决了上文中的第四个缺点，即键名重复输入问题。如下：
